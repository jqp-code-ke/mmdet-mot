...
def eval(coco, outputs, con_thres, iou_thres, img_path):
    img_list = os.listdir(img_path)
    img = np.load(img_path + '/' + img_list[0])[:,:,0:9]
    img_w, img_h, _ = img.shape

    tp = fn = fp = 0

    for i in tqdm(range(len(img_list)), ascii=True):
...
def vis(coco, outputs, root_path, con_thres, iou_thres, stride=250,
        datasets=['val'],
        load_path='H:/Liang/Datas/COCO/Jilin'):
    n_dataset = 0
    save_id = 1
    tp, fn, fp = 0, 0, 0
    for dataset in datasets:
        ...
        for i in range(len(img_list)):
            ...
            img = np.load(img_path + '/' + img_list[i])[:, :, 0:9]
...
def main():
    ...
    coco = COCO(annotation_file=args.gt)

    with open(args.pred, 'r', encoding='utf-8') as load_f:
        strF = load_f.read()
        if len(strF) > 0:
    ...
    # max_con_thres = 0
    # for iter in range(4):
    #     print('con: %.2f' % con_thres, '  ', end=' ')
    #     f1 = eval(con_thres=con_thres, iou_thres=iou_thres, coco=coco, outputs=outputs)
    #     if f1 < 0.1:
    #         break
    #     if max_f1 - f1 > 0.01:
    #         break
    #     if f1 > max_f1:
    #         max_f1 = f1
    #         max_con_thres = con_thres
    #     con_thres += 0.1
    # print('max_con: %.2f, max_f1: %.5f' % (max_con_thres, max_f1))
    eval(con_thres=con_thres, iou_thres=iou_thres, coco=coco, outputs=outputs, img_path=args.images)
