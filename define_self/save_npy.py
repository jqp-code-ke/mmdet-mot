...
def save_data(id, labels, save_paths, img_paths, name='8450-2180'):
    print(id)
    img = [cv2.imread(os.path.join(img_paths, str(ind) + '.png')) for ind in [id - 1, id, id + 1]]
...
def setting(locations, mode, video='4', save_video=None, delect=True):
    ...
    data_paths = r'E:/jqp/data/MOT/SkySat/{}'.format(video)
    label_paths = r'E:/jqp/data/MOT/SkySat/002/gt/gt.txt'
    save_paths = r'E:/jqp/data/MOT/SkySat/{}/{}'.format(save_video, mode)
    ...
    if mode == 'train':
        for location in locations:
            ...
            labels = np.load(label_paths, allow_pickle=True)['label'].item()
            id = np.arange(1, max(labels.keys()), 1)
